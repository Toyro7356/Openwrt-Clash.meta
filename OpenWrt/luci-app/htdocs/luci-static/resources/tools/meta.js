...
return baseclass.extend({
    ...
    clearAppLog: function () {
        return fs.exec_direct('/usr/libexec/meta-call', ['clear_log', 'app']);
    },

    clearCoreLog: function () {
        return fs.exec_direct('/usr/libexec/meta-call', ['clear_log', 'core']);
    },
    ...
    updateSubscription: function (section_id) {
        return fs.exec_direct('/usr/libexec/meta-call', ['subscription', 'update', section_id]);
    },

    status: async function () {
        try {
            return (await this.callServiceList('meta'))['meta']['instances']['meta']['running'];
        } catch (ignored) {
            return false;
        }
    },
    ...
    reload: function () {
        return fs.exec_direct('/usr/libexec/meta-call', ['service', 'reload']);
    },

    restart: function () {
        return fs.exec_direct('/usr/libexec/meta-call', ['service', 'restart']);
    },

    appVersion: function () {
        return L.resolveDefault(fs.exec_direct('/usr/libexec/meta-call', ['version', 'app']), _('Unknown'));
    },

    coreVersion: function () {
        return L.resolveDefault(fs.exec_direct('/usr/libexec/meta-call', ['version', 'core']), _('Unknown'));
    },

    callMihomoAPI: async function (method, path, params, body) {
        const running = await this.status();
        if (running) {
            const apiPort = uci.get('meta', 'mixin', 'api_port');
            const apiSecret = uci.get('meta', 'mixin', 'api_secret');
    ...
    openDashboard: async function () {
        const running = await this.status();
        if (running) {
            const uiName = uci.get('meta', 'mixin', 'ui_name');
            const apiPort = uci.get('meta', 'mixin', 'api_port');
            const apiSecret = encodeURIComponent(uci.get('meta', 'mixin', 'api_secret'));
            let url;
            if (uiName) {
    ...
    getUsers: function () {
        return fs.lines('/etc/passwd').then(function (lines) {
            return lines.map(function (line) { return line.split(/:/)[0] }).filter(function (user) { return user !== 'root' && user !== 'meta' });
        });
    },

    getGroups: function () {
        return fs.lines('/etc/group').then(function (lines) {
            return lines.map(function (line) { return line.split(/:/)[0] }).filter(function (group) { return group !== 'root' && group !== 'meta' });
        });
    },
})
