...
if [ -x "/bin/opkg" ]; then
	...
	# add feed
	echo "add feed"
	if (grep -q meta /etc/opkg/customfeeds.conf); then
		sed -i '/meta/d' /etc/opkg/customfeeds.conf
	fi
	echo "src/gz meta $feed_url" >> /etc/opkg/customfeeds.conf
	# update feeds
	echo "update feeds"
	opkg update
	...
	# add key
	# echo "add key"
	# curl -s -L -o "/etc/apk/keys/meta.pem" "$repository_url/public-key.pem"
	# add feed
	echo "add feed"
	if (grep -q meta /etc/apk/repositories.d/customfeeds.list); then
		sed -i '/meta/d' /etc/apk/repositories.d/customfeeds.list
	fi
